
package au.net.electronichealth.ns.simd.tls.v2010;

import javax.xml.ws.WebFault;
import au.net.electronichealth.ns.smd.core.types.standarderror.V2010.StandardErrorType;


/**
 * This class was generated by the JAX-WS RI.
 * JAX-WS RI 2.2.3-b01-
 * Generated source version: 2.2
 * 
 */
@WebFault(name = "standardError", targetNamespace = "http://ns.electronichealth.net.au/wsp/xsd/StandardError/2010")
public class StandardErrorMsg
    extends Exception
{

    /**
     * Java type that goes as soapenv:Fault detail element.
     * 
     */
    private StandardErrorType faultInfo;

    /**
     * 
     * @param message
     * @param faultInfo
     */
    public StandardErrorMsg(String message, StandardErrorType faultInfo) {
        super(message);
        this.faultInfo = faultInfo;
    }

    /**
     * 
     * @param message
     * @param faultInfo
     * @param cause
     */
    public StandardErrorMsg(String message, StandardErrorType faultInfo, Throwable cause) {
        super(message, cause);
        this.faultInfo = faultInfo;
    }

    /**
     * 
     * @return
     *     returns fault bean: au.net.electronichealth.ns.smd.core.types.standarderror.V2010.StandardErrorType
     */
    public StandardErrorType getFaultInfo() {
        return faultInfo;
    }

}
